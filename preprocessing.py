...
def convert_brats_folder(in_folder, out_folder, truth_name='seg', no_bias_correction_modalities=None):
    for name in config["all_modalities"]:
        try:
            ...
            if name == 't1ce':
                ...
                with open('brseg.nii.gz', 'w') as fp: 
                    pass
            #else:
                #raise error
               
        out=os.path.basename(out_folder) #not in isensee added to fit in xue feng
        print("out",out)
        #out_file = os.path.abspath(os.path.join(out_folder, name + ".nii.gz")) isensee code
        out_file = os.path.abspath(os.path.join(out_folder,out+"_"+ name + ".nii.gz")) #xue feng
    ...
    #out_file = os.path.abspath(os.path.join(out_folder,out+"_"+ "seg.nii.gz")) #feng
    #out_file = os.path.abspath(os.path.join(out_folder, "truth.nii.gz")) isensee
    #shutil.copy(truth_file, out_file)
    check_origin(out_file, get_image(in_folder, config["all_modalities"][0]))

if __name__ == "__main__":
    convert_brats_data("input","Validation2021")
