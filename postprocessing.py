...
from isensee_postprocessevaluator import aggregate_scores
from scipy.ndimage import label
import SimpleITK as sitk
#from nnunet.utilities.sitk_stuff import copy_geometry
from isensee_postprocessfile import *
import shutil
...
def remove_all_but_the_largest_connected_component(image: np.ndarray, for_which_classes: list, volume_per_voxel: float,
    ...
    for c in for_which_classes:
        ...
        print("num_objects and c",num_objects,c)  
        # collect object sizes
        object_sizes = {}
        for object_id in range(1, num_objects + 1):
            object_sizes[object_id] = (lmap == object_id).sum() * volume_per_voxel
        #print("object_sizes and c",object_sizes,c)
        ...
        if num_objects > 0:
            ...
            #print("kept_size and c",kept_size[c],c)
            for object_id in range(1, num_objects + 1):
...
def apply_postprocessing_to_folder(input_folder: str, output_folder: str, for_which_classes: list,
                                   min_valid_object_size:dict, num_processes=8):
    """
    ...
    #print("min_valid_object_size",min_valid_object_size)
...
if __name__ == "__main__":
    ...
    for_which_classes = None#[1,2,4]
    min_valid_object_size = dict({1:32,2:32,4:32})
    apply_postprocessing_to_folder(input_folder, output_folder, for_which_classes,min_valid_object_size)