...
config["patch_shape"] = None # switch to None to train on the whole image (64 ,64, 64)
config["labels"] = (1, 2, 4)  # the label numbers on the input image
config["n_base_filters"] = 20 # these are doubled after each downsampling
...
config["n_epochs"] = 500  # cutoff the training after this many epochs earlier was 300
...
config["training_patch_start_offset"] = (16, 16, 16)  # randomly offset the first patch index by up to this offset default was (16, 16, 16)
...
def fetch_brats_2020_files(modalities, group="Training", include_truth=True, return_subject_ids=False):
    ...
    ABS_PATH = ''
    for subject_dir in glob.glob(os.path.join(ABS_PATH, "TrainingData/data", "*")):
...
"""
#for validation dataset, run predict.py
def fetch_brats_2020_files(modalities, group="Validation", include_truth=False, return_subject_ids=False):
    training_data_files = list()
    subject_ids = list()
    modalities = list(modalities)
    if include_truth:
        modalities = modalities + ["seg"]
    ABS_PATH = ''
    for subject_dir in glob.glob(os.path.join(ABS_PATH, "Validation/data", "*")):
    #for subject_dir in glob.glob(os.path.join(os.path.dirname(__file__), "brats/data", "*","*").format(group)):
        subject_ids.append(os.path.basename(subject_dir))
        subject_files = list()
